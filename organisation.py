...
def variability(center_x, center_y, size, time_vector, domain_size, subdomains, n_bins, min_size, max_size, log_binning, inbetween_subs, show_subs):
    ...
    for timestep in time_vector:
        ...
        if show_subs:
            ...
            for i in range(n_bins):
                ...
                im1 = ax[0].plot(np.log(L),y_axis[:,i], '-o', label='%i m - %i m' %(int(l[i]),int(l[i+1])-1))
                im2 = ax[1].plot(x_axis,y_axis[:,i], '-o', label='%i m - %i m' %(int(l[i]),int(l[i+1])-1))
    ...
    if (len(time_vector) > 1):
        ...
        for i in range(len(f[:,0,0])):
            color = plt.cm.YlOrRd((i+1)/(len(f[:,0,0])+1))
            plt.plot(f[i,0,:],'-', linewidth=1, label='%i m - %i m'%(int(l[i]),int(l[i+1])-1), color=color)
            
        plt.plot(avg_slope,'-', linewidth=2, label='avg. (all sizes)')
        plt.legend()
        plt.xlabel('time')
        plt.ylabel('slope')
